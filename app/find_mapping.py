...
class Lexconn:
    ...
    @staticmethod
    def __one_hot_encoding(ar, range):
        values = ar
        values.shape = (ar.shape[0], 1)
        one_hot = ar == np.arange(range)
    ...
    def get_mapping(self):
        ...
        sdc_e = np.dot(sdc, self.__emission)
        m_logits = tf.Variable(np.random.rand(self.__pdtb_cnt, self.__rst_cnt).astype(np.float32))
